...
尝试使用 tesseract OCR 模型识别 IPA 字符，加速录入过程。失败过程记录。 <!-- more -->
...
- `-l` 指定语言（这里是中文简体）
- `--psm` 指定模式，这里 7 表示单行
不带任何参数输出往往都能看到解释信息，因此下面就只提紧要部分了。
...
在 [tessdata_best](https://github.com/tesseract-ocr/tessdata_best) 上下载对应的各语言最佳效果模型。
